...
export function parseSevenHtml(rawHtml, q, storeId = '', sourceUrl = '') {
  ...
  while ((match = pattern.exec(text)) && products.length < 40) {
    ...
    products.push(normalizeProduct('seven', {
      name,
      price: match[2],
      promotion: match[3] || '',
      storeId,
...
export async function searchSeven({ q, storeId = '' }) {
  const sourceUrl = `https://pyony.com/brands/seven/?search=${encodeURIComponent(q)}`;
  const html = await fetchText(sourceUrl, { timeout: 15000 });
  return { retailer: 'seven', sourceUrl, products: parseSevenHtml(html, q, storeId, sourceUrl) };
}
